...
// -------------------------- Creación ------------------------------------
    ...
    exports.crearInscripcion = async (req, res) => {
        try {
            ...
            const {grupo, estudiante} = req.body || {};

           
            const nuevaInscripcion = await inscripcionService.crearInscripcion({grupo, estudiante});
...
// -------------------------- Modificación ------------------------------------
    ...
    exports.actualizarInscripcion = async (req, res) => {
        ...
        try {
            ...
            const InscripcionActualizada = await inscripcionService.cambiarEstadoInscripcion(id, nuevoEstado);

            if (!InscripcionActualizada) {
                // Si el servicio devuelve null (porque el findByPk falló)
                return res.status(404).json({
                    error: true,
                    message: `Inscripción con ID ${id} no encontrada.`
                });
            }

            // Respuesta de éxito (200 OK)
            res.status(200).json({
                message: `Inscripción ${id} actualizada exitosamente.`,
...
// -------------------------- Obtención ------------------------------------
    ...
    exports.obtenerInscripcionPorId = async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({
                error: true,
                message: "Error al obtener la inscripción: " + error.message
            });
        }
    };
    ...
    exports.buscarInscripciones = async (req, res) => {
        ...
        try {
            ...
            const inscripcionesEncontradas = await inscripcionService.buscarInscripciones(criteriosBusqueda);
