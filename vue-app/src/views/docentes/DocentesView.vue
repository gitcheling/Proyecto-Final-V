<template>
  ...
  <div class="account-type-manager">
    ...
    <div class="table-responsive">
        ...
        <thead class="header-personalizado">
            <tr>
                ...
                <th class="text-center col-fecha_creacion">Fecha de contratación como docente</th>
                <th class="text-center col-fecha_actualizacion">Última modificación</th>
                <th class="text-center col-estado">Estado</th>
                <th class="text-center col-puede_inscribirse">¿Puede ser asignado a nuevos cursos?</th>
                <th class="text-center col-puede_inscribirse">¿Puede recibir pagos?</th>
        ...
        <tbody>
            ...
            <tr v-else-if="teacherTypes.length === 0">
                <td colspan="10" class="text-center">
                    <span class="no-results-message">No se encontraron docentes. Intenta ajustar los filtros.</span>
                </td>
            </tr>
            <tr v-else v-for="teacher in teacherTypes" :key="teacher.id">
                <td>{{`${teacher.entidad.prefijo.letra_prefijo} - ${student.entidad.numero_identificacion}` }}</td>
                ...
                <td>{{ teacher.estado.puede_inscribirse  }}</td>
    ...
    <!-- Modal para crear o editar un estudiante -->
    <TeacherFormModal
...
<script setup>
    ...
    // ----------------------------------- Variables ----------------------------------------
        ...
        // Almacena el objeto del estudiante que se está editando. Es 'null' en el modo creación y se pasa como argumento a la función respectiva
            ...
            */   
            const addTeacher = async (newTeacherData) => {
                try {
                    ...
                    const response = await api.post('/Docentes/CrearDocente', newTeacherData);
                    ...
                    error('Error al registrar al docente', `${err.response?.data?.message || 'Error de servidor.'}`);
                }
            };


            /**
             * Maneja el evento 'update-status' del modal llamando a la API.
             */
            const updateStatus = async (updatedData) => {
                    ...
                    try {
                        ...
                        const response = await api.put(`/Docentes/Modificar/${updatedData.id}`, updatedData);

                        exito('Éxito', 'Docente modificado correctamente.');
                        ...
                        error('Error al modificar al docente', `${err.response?.data?.message || 'Error de servidor.'}`);
      ...
      // ----------------------------------- Modal ----------------------------------------
        ...
        */
        const openModal = (teacher = null) => {
          ...
          /* En modo edición se tendría el objeto del docente (es decir, el objeto del array), y se usará para crear una copia
