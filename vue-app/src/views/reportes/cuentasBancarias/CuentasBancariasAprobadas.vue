<template>
  <div class="analytics-manager">
    ...
    <div class="chart-container hover-lift p-4">
        <h3>Aprobación de Cuentas Bancarias</h3>
...
// ------------------------------------- Variables  ------------------------------------
    // Rutas
        ...
        const rutaContar = `${rutaBase}ContarAprobadas` 
...
// ------------------------------------- Funciones  ------------------------------------
    ...
    // ----------------------------------- API y Flujo de Datos ----------------------------------------
         ...
         */
        const loadAccountsForChart = async (currentFilters = {}) => {
           ...
           isLoadingChart.value = true;
            try {
                ...
                const response = await api.get(rutaContar, { 
                    params: { ...validFilters, concepto: "aprobados" }
                }); 
    ...
    // ----------------------------------- Watchers ----------------------------------------
        ...
        watch(
            ...
            (newFilters) => {
                ...
                searchTimeout = setTimeout(() => {
                    ...
                    loadAccountsForChart(newFilters);
                }, 300); 
            }, 
            { 
                deep: true, 
        ...
        onMounted(async () => {
            ...
            fetchBancos();
            fetchEstados();
            await loadAccountsForChart(filters.value);
        
        });


        // Detecta si 'aprobadosDesde' fue cambiado manualmente
        watch(() => filters.value.aprobadosDesde, (newValue) => {
        ...
        // Detecta si 'aprobadosHasta' fue cambiado manualmente
        watch(() => filters.value.aprobadosHasta, (newValue) => {
    ...
    // ----------------------------------- Utilidad ----------------------------------------   
        ...
        if (!year) {
                ...
                filters.value.aprobadosDesde = '';
                filters.value.aprobadosHasta = '';
            } else {
                // Si el año es válido (un número), aplicamos el rango normal.
                filters.value.aprobadosDesde = `${year}-01-01`;
                filters.value.aprobadosHasta = `${year}-12-31`;
            }
        };
