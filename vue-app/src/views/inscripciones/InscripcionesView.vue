<template>
  ...
  <div class="account-type-manager">
    ...
    <div 
    v-if="registrationsTypes.length > 0 && !isLoadingTable" 
    ...
    <div class="table-card-wrapper hover-lift">
        <div class="table-responsive">
            ...
            <thead class="header-personalizado">
                <tr>
                    <th class="text-center col-nombre">Código Estudiantil</th>
                    <th class="text-center col-nombre">Nombre del Estudiante</th>
                    <th class="text-center col-curso">Nombre del Grupo</th>
                    <th class="text-center col-docente">Docente del Grupo</th>
                    <th class="text-center col-modalidad">Periodo</th>
            ...
            <tbody>
                ...
                <tr v-if="registrationsTypes.length > 0" v-for="registration in registrationsTypes" :key="registration.id">
                    <td class="">{{ registration.estudiante.codigo_estudiantil }}</td>
                    <td class="">{{ registration.estudiante.entidad.nombre }} {{ registration.estudiante.entidad.apellido }} ({{ registration.estudiante.entidad.prefijo }}-{{ registration.estudiante.entidad.numero_identificacion }})</td>
                    <td class="">{{ registration.grupo.nombre }}</td>
                    <td class="">{{ registration.grupo.docente.entidad.nombre }} {{ registration.grupo.docente.entidad.apellido }} ({{ registration.grupo.docente.entidad.prefijo }}-{{ registration.grupo.docente.entidad.numero_identificacion }})</td>   
                    <td class="">{{ registration.grupo.periodo.nombre }}</td>
                    <td class="">{{ registration.estado.nombre }}</td>
                    ...
                    <td class=" text-center">
                        <div class="d-flex flex-row flex-nowrap justify-content-center">
                             ...
                             <button 
                                ...
                                @click="showDetailsModal(registration)" 
    ...
    <div 
        v-if="!isLoadingTable && registrationsTypes.length === 0" 
        class="text-center py-5 mb-5"
    >
        <div class="no-results-center-badge">
            <i class="bi bi-x-circle-fill me-2"></i> No se encontraron inscripciones con los filtros aplicados.
            <p class="mt-2 mb-0 text-muted">Intenta ajustando o limpiando los filtros para ver la lista completa.</p>
        </div>
    </div>

    <!-- Modal para crear una inscripción -->
    <FormModal
...
<script setup>
    ...
    // ----------------------------------- Importaciones ----------------------------------------
        ...
        import DataDetailsModal from './InscripcionDetailsModal.vue';
    ...
    // ----------------------------------- Variables ----------------------------------------
        ...
        // Rutas
            ...
            // Crear inscripción
            const rutaCrear = `${rutaBase}CrearInscripcion`
        ...
        const registrationsTypes = ref([]);

        // Propiedad computada para saber que mensaje se pondrá en la cantidad de resultados encontrados
        const resultsText = computed(() => {
            const count = registrationsTypes.value.length;
            if (count === 1) {
                return `🔍 Se encontró ${count} inscripción con los filtros aplicados.`;
            } else {
                return `🔍 Se encontraron ${count} inscripciones con los filtros aplicados.`;
            }
        });
      ...
      // ----------------------------------- API ----------------------------------------
            ...
            const loadData = async (currentFilters = {}) => {
                ...
                try {
                    ...
                    registrationsTypes.value = response.data.data; 

                } catch (err) {
                    error('Error de Servidor', 'No se pudieron obtener los datos de las inscripciones. Intente de nuevo.');
                } finally {
            ...
            /**
            * Maneja el evento 'add-registration' del modal llamando a la API.
            */   
            const addRegistration = async (newData) => {
                try {

                    // 1. Llama a la API (POST) para creación
                    const response = await api.post(rutaCrear, newData);

                    exito('Éxito', 'Inscripción realizada correctamente, esperando por el pago.');
